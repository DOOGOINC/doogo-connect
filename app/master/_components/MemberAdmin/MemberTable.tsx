...
function KakaoBadge() {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-[#FEE500] px-2.5 py-1 text-xs font-bold text-[#3B1E1E]">
...
function EmailBadge() {
  return <span className="inline-flex items-center rounded-full bg-[#EEF5FF] px-2.5 py-1 text-xs font-bold text-[#0064FF]">이메일</span>;
}
...
export function MemberTable({
  ...
  return (
    <div className="mb-5 flex-1 overflow-x-auto">
      <table className="w-full min-w-[1600px] table-fixed border-collapse text-[14px]">
        <thead className="sticky top-0 z-10 bg-[#F9FAFB] shadow-sm">
          <tr className="text-left text-[12px] font-bold uppercase tracking-wider text-[#6a7282]">
            <th className="w-[130px] cursor-pointer px-3 py-2 transition-colors hover:bg-gray-50" onClick={() => toggleSort("full_name")}>
              <div className="flex items-center gap-1">이름 <SortIcon field="full_name" sortBy={sortBy} /></div>
            </th>
            <th className="w-[200px] cursor-pointer px-3 py-2 transition-colors hover:bg-gray-50" onClick={() => toggleSort("email")}>
            ...
            <th className="px-3 py-2">가입방식</th>
            <th className="w-[120px] cursor-pointer px-3 py-2 transition-colors hover:bg-gray-50" onClick={() => toggleSort("role")}>
              <div className="flex items-center gap-1">권한 <SortIcon field="role" sortBy={sortBy} /></div>
            </th>
            <th className="w-[120px] cursor-pointer px-3 py-2 transition-colors hover:bg-gray-50" onClick={() => toggleSort("created_at")}>
              <div className="flex items-center gap-1">가입일 <SortIcon field="created_at" sortBy={sortBy} /></div>
            </th>
            <th className="w-[160px] cursor-pointer px-3 py-2 transition-colors hover:bg-gray-50" onClick={() => toggleSort("updated_at")}>
            ...
            members.map((member) => {
              ...
              return (
                <tr key={member.id} className="align-middle transition-colors hover:bg-[#FCFDFF]">
                    ...
                    {member.business_attachment_url ? (
                      <div className="flex flex-wrap items-center gap-1.5">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center rounded-full border border-[#cfe0ff] bg-[#eef5ff] px-2.5 py-1 text-xs font-bold text-[#0064FF] transition hover:bg-[#e1eeff]"
                        >
                          확인
                        </a>
                        <button
                          ...
                          className="inline-flex items-center rounded-full border border-[#d8e0e8] bg-white px-2.5 py-1 text-xs font-bold text-[#4E5968] transition hover:border-[#b9c7d6] hover:bg-[#f8fafc]"
                        >
                          다운로드
                        </button>
                      </div>
                  ...
                  <td className="px-3 py-2">
                    <span
                      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${member.role === "master"
                    ...
                    >
                      {member.role === "master" ? "마스터" : member.role === "manufacturer" ? "제조사" : "의뢰자"}
                    </span>
                  </td>
                  ...
                  <td className="px-3 py-2 text-right">
                    <div className="inline-flex items-center gap-1.5">
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-lg border border-[#D8E0E8] bg-white px-2.5 py-1 text-xs font-bold text-[#4E5968] transition hover:border-[#0064FF] hover:text-[#0064FF] disabled:cursor-not-allowed disabled:border-[#F2C94C] disabled:bg-[#FFF8D9] disabled:text-[#8A6D1F]"
                      >
                      ...
                      <select
                        ...
                        className="h-8 cursor-pointer rounded-lg border border-[#E5E8EB] bg-white px-2 py-1 text-xs font-bold outline-none hover:border-[#0064FF] disabled:cursor-not-allowed disabled:bg-gray-50"
                      >
