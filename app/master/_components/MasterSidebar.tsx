...
import { Factory, LogOut, MessageSquare, Settings, ShieldCheck, Users } from "lucide-react";
  ...
  return (
    <aside className="flex min-h-screen w-56 flex-shrink-0 flex-col border-r border-gray-200 bg-white">
      ...
      <nav className="flex-1 overflow-y-auto py-3">
        <button
          type="button"
          className="flex w-full items-center gap-2.5 px-5 py-2.5 text-sm font-semibold text-gray-700"
        >
          ...
          {MASTER_MENU.map((item) => (
            <button
              ...
              className={`flex w-full items-center gap-2.5 px-6 py-2 text-sm transition-colors ${
                activeTab === item.id
                  ? "bg-blue-50 font-semibold text-[#0064FF]"
                  : "text-gray-600 hover:bg-gray-50"
              }`}
            >
              <item.icon
                className={`h-3.5 w-3.5 ${
                  activeTab === item.id ? "text-[#0064FF]" : "text-gray-400"
                }`}
              />
              {item.label}
            </button>
