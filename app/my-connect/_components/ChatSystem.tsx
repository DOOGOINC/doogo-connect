  ...
  const refreshRooms = useCallback(async () => {
    ...
    const roomQuery =
      viewMode === "manufacturer" && effectiveManufacturerId
        ? supabase.from("chat_rooms").select("*").eq("manufacturer_id", effectiveManufacturerId)
        : supabase.from("chat_rooms").select("*").eq("client_id", userId);
    ...
    const manufacturerIds = Array.from(new Set(roomsData.map((room) => room.manufacturer_id)));
