...
export function DesignServiceCatalogManager({ manufacturerId, currencyCode }: { manufacturerId: number; currencyCode: CurrencyCode }) {
  ...
  const handleSave = async () => {
    ...
    if (!isEdit && items.some((item) => item.id === nextId)) {
      alert("이미 사용 중인 서비스 ID입니다. 다른 ID를 입력해주세요.");
      return;
    }
    ...
    const result = isEdit
      ? await supabase.from("manufacturer_design_services").update(payload).eq("id", editingId)
      : await supabase.from("manufacturer_design_services").insert({ id: nextId, ...payload });
    setSaving(false);

    if (result.error) return alert(`저장 실패: ${result.error.message}`);
    await loadItems();
    resetForm();
    setToastMessage(isEdit ? "디자인 서비스가 수정되었습니다." : "디자인 서비스가 추가되었습니다.");
  };
    ...
    <section className="rounded-[16px] border border-[#E5E8EB] bg-white p-6 shadow-sm md:p-8">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[14px] text-[#8B95A1]">개별 서비스 항목과 가격을  관리합니다.</p>
        </div>
      ...
      <CatalogModal
        open={open}
        onClose={resetForm}
        badge={isEdit ? "서비스 수정" : "서비스 추가"}
        title={isEdit ? "디자인 서비스를 수정합니다" : "디자인 서비스를 추가합니다"}
        description=""
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            <label className="text-[13px] font-semibold text-[#4E5968]">서비스 ID</label>
            <input
              value={form.id}
              disabled={isEdit}
              onChange={(event) => setForm((prev) => ({ ...prev, id: event.target.value }))}
              className={inputClassName}
              placeholder="예: SERVICE-001"
            />
          </div>
          <div className="space-y-2">
            <label className="text-[13px] font-semibold text-[#4E5968]">서비스명</label>
            <input
              value={form.name}
              onChange={(event) => setForm((prev) => ({ ...prev, name: event.target.value }))}
              className={inputClassName}
              placeholder="예: 로고 인쇄 디자인"
            />
          </div>
          <div className="space-y-2 sm:col-span-2">
            ...
            <textarea
              ...
              placeholder="서비스 구성이나 범위를 간단히 설명하세요."
            />
          </div>
          <div className="space-y-2">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b-2 border-[#F2F4F6] text-left">
              ...
              <th className="pb-4 text-[13px] font-semibold text-[#8B95A1]">서비스 ID / 이름</th>
