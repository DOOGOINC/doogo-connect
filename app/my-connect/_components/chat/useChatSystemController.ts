...
import { CHAT_FILE_ACCEPT, getChatFileValidationMessage, isAllowedChatFile } from "./fileConstraints";
import type {
  ChatMessageRow,
  ChatMessageView,
  ChatRoomMemoValue,
...
import { formatMessageTime, formatRelativeTime, getAvatarInitial, getPresenceLabel, getProfileDisplayName, isRecentlyOnline } from "./utils";
...
export function useChatSystemController({
  userId,
  viewMode,
  initialRoomId = "",
}: {
  ...
  const loadChatParticipants = useCallback(async (roomIds: string[]) => {
    if (!roomIds.length) {
      return {
        profiles: [] as ProfileRow[],
        requestContacts: [] as Array<{
          client_id: string;
          manufacturer_id: number;
          contact_name: string | null;
          updated_at: string | null;
    ...
    try {
      const response = await authFetch("/api/chat/participants", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        return { profiles: [] as ProfileRow[], requestContacts: [] };
      }

      const payload = (await response.json()) as ChatParticipantsPayload;
      return {
        profiles: payload.profiles || [],
        requestContacts: payload.requestContacts || [],
      };
    } catch {
      return { profiles: [] as ProfileRow[], requestContacts: [] };
    }
  ...
  const deleteRoomMemo = useCallback(
    async (roomId: string) => {
  ...
  const refreshRooms = useCallback(async () => {
    ...
    try {
      ...
      const [{ profiles, requestContacts }, { data: unreadMessages }, nextMemoMap] = await Promise.all([
        loadChatParticipants(roomIds),
      ...
      (((requestContacts as Array<{ client_id: string; contact_name: string | null }> | null) || []) as Array<{
        client_id: string;
        contact_name: string | null;
      }>).forEach((row) => {
        const contactName = row.contact_name?.trim();
        if (row.client_id && contactName && !requestContactMap.has(row.client_id)) {
          requestContactMap.set(row.client_id, contactName);
        }
      });
      ...
      const nextRooms: ChatRoomView[] = roomsData.map((room) => {
        if (viewMode === "manufacturer") {
          ...
          const counterpartName = getProfileDisplayName(clientProfile, requestContactMap.get(room.client_id) || "의뢰자");
  ...
  }, [initialRoomId, loadChatParticipants, loadPresenceMap, loadRoomMemoMap, manufacturerId, userId, viewMode]);
  ...
  const sendMessage = useCallback(
    ...
    [refreshMessages, refreshRooms, replaceMessage, selectedRoomId, toMessageView, touchPresence]
  );
  ...
  return {
    ...
    memoMap,
    messageInput,
    messageListRef: messageListRef as RefObject<HTMLDivElement | null>,
    messages,
    ...
    selectedRoom,
    selectedRoomId,
    sendCurrentMessage: () => void sendMessage({ content: messageInput }),
    setRoomFilter,
    setSearchQuery,
  };
}
