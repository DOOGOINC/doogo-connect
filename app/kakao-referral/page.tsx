...
export default function KakaoReferralPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "카카오 회원가입 저장 중 오류가 발생했습니다.");
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <section className="w-full p-8">
        <div className="mb-8">
          ...
          <p className="mt-3 text-sm leading-6 text-[#6B7684]">
            카카오 로그인 회원은 이 단계에서 이름과 연락처를 한 번만 입력하면 됩니다.<br /> 추천인 코드는 선택 입력이며 최초 1회만 적용됩니다.
          </p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            ...
            {appliedReferralCode ? (
              ...
              <p className="mt-2 text-[12px] font-medium text-[#6B7684]">추천인 코드는 회원가입 시 최초 1회만 적용됩니다.</p>
            )}
          </div>
        </div>
