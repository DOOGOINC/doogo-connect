...
export async function POST(request: Request, context: Params) {
  try {
    ...
    if (room.room_type !== "support") {
      throw new Error("고객센터 요청만 수락할 수 있습니다.");
    }

    if (room.approval_status === "approved" && room.master_profile_id && room.master_profile_id !== user.id) {
      throw new Error("이미 다른 마스터가 수락한 요청입니다.");
    }
