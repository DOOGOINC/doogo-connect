...
function getSettledAt(monthKey: string) {
  const [year, month] = monthKey.split("-").map(Number);
  const settledDate = new Date(year, month, 5);

  return settledDate.toISOString();
}
...
export async function GET(request: Request) {
  try {
    ...
    for (const order of ((orderRows as SettlementOrderRow[] | null) || []) as SettlementOrderRow[]) {
      ...
      const current = historyMap.get(month) || {
        month,
        nzdBaseAmount: 0,
        nzdProfitAmount: 0,
        krwBaseAmount: 0,
        krwProfitAmount: 0,
        settledAt: getSettledAt(month),
    ...
    const rows = [...historyMap.values()].sort((a, b) => b.month.localeCompare(a.month));
